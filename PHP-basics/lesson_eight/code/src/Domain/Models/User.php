...
class User
{
    ...
    public static function getAllUsersFromStorage(?int $limit = null): array
    {
        ...
        foreach ($result as $item) {
            // Преобразуйте строку даты в timestamp
            $birthdayTimestamp = strtotime($item['user_birthday']);
            $user = new User($item['id_user'], $item['user_name'], $item['user_lastname'], $birthdayTimestamp);
            $users[] = $user;
        }
